...
# %%##################################################################
# Read active authors
ACTIVE_AUTHORS_FILE = 'AU_GAT_ACTIVE.txt'
...
BASE_FILE_NAME = 'Autorship_attribution_GAT_results_'
...
try:
    ...
    if doGCAT:
        ...
        # \\\ Architecture parameters
        hParamsGCAT['dimNodeSignals'] = [1, 32]  # Features per layer
        hParamsGCAT['nFilterTaps'] = [5]  # Number of filter taps per layer
    ...
    for combination in combinations:
        hParamsGAT['nAttentionHeads'] = [combination]  # Number of attention heads
        ...
        training_results[str(combination)] = {"acc": list(accBest['GAT']), "f1": list(f1_best['GAT']),
                                              "auc": list(roc_best['GAT'])}
