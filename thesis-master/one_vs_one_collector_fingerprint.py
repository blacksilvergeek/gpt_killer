...
NO_OF_SPLITS = 10
ONE_VS_ALL_CRASH_FILE = "results/one_vs_one/One_vs_one_fingerprint_crash_dump_{0}_{1}.json"
ONE_VS_ONE_RESULTS = "results/one_vs_one/One_vs_one_fingerprint_results_{0}.json"
...
def main():
    ...
    for idx, name in enumerate(all_author_names):
        ...
        try:
            ...
            svc_results[name] = collect_svc(name, most_similar_name, indices)
            logging.info('SVM results successfully collected: {0}'.format(svc_results[name]))

            GCNN_results[name] = collect_gcnn(name, most_similar_name, phi, indices)
...
if __name__ == '__main__':
    logging.basicConfig(filename='one-vs-one-fingerprint.log',
