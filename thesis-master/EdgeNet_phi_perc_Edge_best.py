...
HYPER_PARAM_FILE = 'model_comparison_df_edge.json'
...
# %%##################################################################
# Read active authors
ACTIVE_AUTHORS_FILE = 'AU_edgenet_phi_perc_Edge_ACTIVE.txt'
...
BASE_FILE_NAME = 'EdgeNet_phi_perc_Edge_best_results_'
...
try:
    ...
    nFeatures, nShifts = ClusterUtils.load_best_hyperparams(authorName, HYPER_PARAM_FILE)  # K: number of shift tap
    ...
    for combination in combinations:
        if str(combination) in list(training_results.keys()):
            ...
            training_results[str(combination)] = {"acc": [], "f1": [], "auc": []}
        ...
        for split in range(len(training_results[str(combination)]['acc']), nDataSplits):
            ...
            if doPolynomialGNN:
                ...
                hParamsPolynomial['F'] = [1, nFeatures]
