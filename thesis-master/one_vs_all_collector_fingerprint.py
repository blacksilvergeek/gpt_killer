...
NO_OF_SPLITS = 10
ONE_VS_ALL_CRASH_FILE = "results/one_vs_all/One_vs_one_fingerprint_crash_dump_{0}_{1}.json"
ONE_VS_ONE_RESULTS = "results/one_vs_all/One_vs_one_fingerprint_results_{0}.json"


def main():
    Path("results/one_vs_all").mkdir(parents=True, exist_ok=True)

    logging.info('Loading data for One-vs-All with fingerprint matrix.')
...
def load_phi(name, phi_matrix_path='EdgeVariGNN_important_words_phi.txt', percentage=0.2):
    with open(phi_matrix_path, 'r') as f:
        ...
        while np.unique(indices_to_keep.flatten()).shape[0] <= 10:
            mult += 0.05
...
if __name__ == '__main__':
    logging.basicConfig(filename='one-vs-All-fingerprint.log',
