...
# %%##################################################################
# Read active authors
ACTIVE_AUTHORS_FILE = 'AU_edgenet_phi_perc_GCNN_ACTIVE.txt'
...
BASE_FILE_NAME = 'EdgeNet_phi_perc_GCNN_best_results_'
...
try:
    ...
    with open('EdgeVariGNN_important_words_phi.txt', 'r') as f:
        ...
        phi_whole = np.array(file[authorName]['phi'])
        phi_matrix = np.zeros(phi_whole.shape)

        mask_array = np.zeros(phi_whole.shape, dtype=bool)
        mask_array[[x[0] for x in file[authorName]['indices']], [x[1] for x in file[authorName]['indices']]] = True

        np.copyto(phi_matrix, phi_whole, where=mask_array)
    ...
    for combination in combinations:
        if str(combination) in list(training_results.keys()):
            ...
            training_results[str(combination)] = list(filter(None, training_results[str(combination)]))

            for idx, item in enumerate(training_results[str(combination)]):
                accBest[hParamsPolynomial['name']][idx] = item
                accLast[hParamsPolynomial['name']][idx] = item
        ...
        for split in range(len(training_results[str(combination)]), nDataSplits):
